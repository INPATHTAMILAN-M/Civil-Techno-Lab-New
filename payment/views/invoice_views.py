...
from ..serializers import (
    ...
    Customer_Serializer1, Sales_mode_Serializer1, Tax_Serializer1,
    Material_Test_Serializer, Material_Serializer2,
    Create_Invoice_Test_Serializer, Invoice_Test_Serializer,
    Invoice_Serializer1, Test_serializer, Pending_Invoice_Serializer,
    Create_Invoice_File_Serializer, Invoice_File_Serializer,
    Expense_Entry_Serializer1, Customer_Serializer_For_Invoice,
    Invoice_Serializer_For_Report, Edit_Invoice_Serializer,
    Invoice_Serializer_For_Print, Customer_Serializer_For_Print,
    Invoice_Test_Serializer_For_Print, Invoice_File_Category_Serializer,
...
class Create_Invoice(APIView):
    ...
    def get_financial_year(self):
        ...
        # Financial year format (e.g., 24-25 for FY 2024-2025)
    ...
    def post(self, request, *args, **kwargs):
        ...
        if serializer.is_valid():
            ...
            invoice_no = f"{invoice_no}/{financial_year}"
            ...
            invoice_img_path  = "media/invoice/invoice_"+str(invoice_no.split('/')[0])+"-" \
                                +str(invoice_no.split('/')[1])+".png"
...
class Invoice_File_Report(APIView):
    ...
    def post(self, request):
        ...
        if customer:
            invoicefiles = invoicefiles.filter(invoice__customer__id=customer)
        if project_name:
            invoicefiles = invoicefiles.filter(Q(invoice__project_name__icontains = project_name))
        if invoice_no:
