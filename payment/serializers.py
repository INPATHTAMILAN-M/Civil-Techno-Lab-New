...
class Invoice_Test_Serializer(serializers.ModelSerializer):
    ...
    def get_without_signature(self,obj): 
        ...
        if obj.signature:   
            ...
            data = data.replace('</td>','',1)
            data = data.replace('</tr>','',1)
            data = data + f'<figure class="table"><table cellpadding="0" cellspacing="1" border="0" style="border:none !important; border-spacing:0.6pt; width:100%"><tr><td width="40%" style="border:none !important;"><p></p><p style="text-align:justify"><strong>M.RAGURAM, ME (STRUCTURAL)., AMIE <br>CHARTERED ENGINEER (AM 1818123)<br>Covai Civil Lab Private Limited </strong></p></td><td width="40%"  style="border:0 !important;"><p><img src="data:image/gif;base64,R0lGODlhAQABAIAAAAAAAP///ywAAAAAAQABAAACAUwAOw==" height="38px" width="38px"></p><p id="dynamic-signature"></p><p style="font-size:12px"><strong style="font-size:12px">'+str(obj.signature)+'<br>'+str(obj.signature.role)+f'</strong></p><p style="font-size:12px"><strong>Covai Civil Lab Private Limited</strong></p>  </td> <td width="20%"  style="border:0 !important;" class="qr-code"> <img src="{settings.BACKEND_DOMAIN}/'+str(obj.invoice_image)+'" ></td> </tr></table></figure>'
            soup = BeautifulSoup(data, 'html.parser')
            first_table = str(soup.select_one("table:nth-of-type(1)"))    
